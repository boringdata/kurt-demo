...
kurt document get 44ea066e  # Partial UUID works
...
| Task | CLI | Python API |
|------|-----|------------|
| List documents | `kurt document list` | `list_documents()` |
| Filter by URL | `--url-prefix https://...` | `url_prefix="https://..."` |
| Get document | `kurt document get <id>` | `get_document(document_id)` |
...
| Issue | Solution |
|-------|----------|
| "Document not found" | Check `kurt document list` or use more UUID chars |
| "Ambiguous ID" | Use more characters: `44ea066eca` instead of `44ea` |
| Metadata is null | Document not fetched yet - run `kurt ingest fetch <id>` |
